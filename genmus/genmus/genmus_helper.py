...
# Code was borrowed from the below open source project and served
#   as the inspiration of the current project.
...
def apply_pedal(note_values, bar_value):
	...
	if sum(note_values) % bar_value != 0 : # This seems to fix a bug; both vars must be equal.
		...
		while(sum(note_values) % counter != 0):
			counter = round(counter +  .1,2) # round to two decimal places to avoid long trails of random error
...
def get_song_data(music_notes, note_values, bar_value, factor, length, decay, sustain_level, sample_rate=44100, amplitude=4096):
	...
	for i in range(len(music_notes)):
		...
		weights = get_adsr_weights(frequencies[i], new_values[i], length, decay, sustain_level)
...
def helper():
	...
	print(printWithColour(BICyan,"\t [+] Setup with Poetry : "), printWithColour(BIYellow,"poetry install"))
...
def openDnaSeq(fastaFile_str):
	...
	if len(seq_dic) == 0:
		print(printWithColour(BIRed, f"\t [+] Bad format or incorrect Fasta file: <<{fastaFile_str}>>"))
		exit()
...
def pairMusicWordWithNote(this_dic, Twinkle_list):
	...
	print("\t Frequencies; ",printWithColour(BIGreen,f"{this_dic}"), "Max Value :", printWithColour(BICyan,f" {myMaxValueThisDic_int}"))
	...
	for v in range(myMaxValueThisDic_int):
		...
		freq_dic[v] = [i for i in this_dic if this_dic[i] == v]   #[i] == myMaxValueThisDic_int for i in this_dic]
...
def makeMusicDemo(names_list, left_hand_notes, left_hand_duration, right_hand_notes = [], right_hand_duration = []):
	...
	sustain_level = 0.1
	right_hand = get_song_data(right_hand_notes, right_hand_duration, 2, factor, length, decay, sustain_level)
...
def makeMusicFromChars(name_str, notes_list, duration_list):
	...
	decay = [0.05,0.02,0.005,0.1]
	sustain_level = 0.1
	data = get_song_data(notes_list, duration_list, 2, factor, length, decay, sustain_level)
